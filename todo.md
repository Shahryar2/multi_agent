...
19. 是否应该区分最终生成的文档和书写过程中的背景效果，作区分，如 问题9.
    重审的原因也可以呈现在前端(在上方的思考框内但是不是很起眼)

< 需要明确的问题 >
    在该思路的基础上，我觉得应该精细化、优化重审逻辑以及重审后实现文档逻辑
    保证分章节文档不割裂、不过分重复；
    重审后新旧内容融合融洽、不割裂；
    且准确化重申修改部分，如何具体将重审后修改的内容完美融入旧内容；
    都是需要明确的问题
< 暂时好像解决 >
    出现了截断问题；
    出现引用不正确问题；
    直接复用部分文档，测试输出文档重审前后看不明显对于出现问题的更正和补充(待核实)
...
22. 知识库的问题，需要构建知识库吗？用于用户上传和保存需要研究的文档？(关于项目定位的问题，有哪些需要舍弃，哪些需要增加的)
    以及数据库以及缓存的问题

23. 提示词问题(避免 提示词注入，需要看下我的提示词中是否存在该问题)

24. https://www.nowcoder.com/discuss/858407685358620672
    参考该链接，我项目是否具备或者说 是否需要完善上述的功能

25. 想要能够多轮对话，但是同时涉及 记忆栈的问题；
    每轮对话可以修改或是自动保存为第一次对话？还是更新为该轮对话的最后一次问题凝练的标题？
    资料库应该怎么处理，每次研究获得的资料都是不一样的，那么资料库应该怎么变动？
    以及是否要区分每次对话 是否都是研究模式？